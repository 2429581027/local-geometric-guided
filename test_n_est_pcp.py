    ...
    if opt.batchSize == 0:
        ...
        if trainopt.arch == 'simple':
            spec = importlib.util.spec_from_file_location("DeepFit", os.path.join(
                os.path.join(opt.logdir, model_name, "DeepFit_pcp.py")))
            DeepFit_pcp = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(DeepFit_pcp)
            regressor = DeepFit_pcp.DeepFit(1, num_points=trainopt.points_per_patch,
        ...
        for batchind, data in batch_enum:
            ...
            with torch.no_grad():
                if trainopt.arch == 'simple' or trainopt.arch == 'res' or trainopt.arch == '3dmfv':
                    ...
                    n_est, n_est_t, beta_pred, weights, trans, trans2, neighbor_normals = regressor(pts_feas)
                    end_time = time.time()

                    n_est_len = torch.max(n_est.new_tensor([sys.float_info.epsilon*100]), n_est.norm(p=2, dim=1, keepdim=True))
                    n_est = n_est / n_est_len
                    #n_est_len = torch.max(n_est.new_tensor([sys.float_info.epsilon*100]), n_est.norm(p=2, dim=1, keepdim=True))

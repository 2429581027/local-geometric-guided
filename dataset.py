...
def load_shape(point_filename, normals_filename, features_filename, curv_filename, pidx_filename):
    ...
    return Shape(pts=pts, kdtree=kdtree, normals=normals, features = features, curv=curvatures, pidx=patch_indices)
...
class Shape():
    def __init__(self, pts, kdtree, normals=None, features=None, curv=None, pidx=None):
        self.pts = pts
        self.kdtree = kdtree
...
class PointcloudPatchDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        neighbor_normals = torch.zeros(self.points_per_patch * len(self.patch_radius_absolute[shape_ind]), 3,
                                dtype=torch.float)
        #pts_features_all = torch.zeros(self.points_per_patch * len(self.patch_radius_absolute[shape_ind]), 18,
                                #dtype=torch.float)
        pts_features = torch.zeros(self.points_per_patch * len(self.patch_radius_absolute[shape_ind]), 18,
                                dtype=torch.float)
        ...
        for s, rad in enumerate(self.patch_radius_absolute[shape_ind]):
            ...
            elif self.center == 'knn':
                patch_pts[start:end, :] = patch_pts[start:end, :] - patch_pts[start:15, :].mean(0)
        ...
        if self.include_features:
            pts_features[start:end, :] = torch.from_numpy(shape.features[patch_point_inds, :])
            
            base = 8

            for i in range(6):
                base = base *2
                t_pts = patch_pts[0:base, :]
                t_center = t_pts.mean(0)
                t_pts = t_pts - t_center
                t_trans,_,_ = torch.svd(torch.t(t_pts))

                pts_features[: , 3*i:3*(i+1)] = torch.mm(pts_features[: , 3*i:3*(i+1)], t_trans)
            
            #feature_id = [3,4,5,9,10,11,15,16,17]
            #pts_features = pts_features_all[: , feature_id]
            #pts_features = pts_features - pts_features[0,:]
        ...
        if self.use_pca:
            ...
            #if self.include_features:
                #pts_features = torch.matmul(pts_features, trans)
        ...
        for pfeat in self.patch_features:
            if pfeat == 'normal':
                ...
                #patch_feats = patch_feats + (pts_features,)
        ...
        pts_feas = torch.cat((patch_pts, pts_features) , 1)
        return (pts_feas,) + patch_feats + (trans,)  + (patch_scale,) #+ (effective_points_num,)
    ...
    def load_shape_by_index(self, shape_ind):
        ...
        features_filename = os.path.join(self.root, self.shape_names[shape_ind]+'.features') if self.include_features else None
        ...
        return load_shape(point_filename, normals_filename, features_filename, curv_filename, pidx_filename)
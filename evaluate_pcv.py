...
parser.add_argument('--normal_results_path', type=str, default='/home/hrzhu/work2021/results/pclouds/PCA_results/PCA_450/SharpFeature', help='Log dir [default: log]')
parser.add_argument('--data_path', type=str, default='/home/hrzhu/work2021/data/data_pcv_one_normal/SharpFeature/', help='Relative path to data directory')
parser.add_argument('--data_path_all_norm', type=str, default='/home/hrzhu/work2021/data/Data-PCV/TrueMat/SharpFeature', help='Relative path to data directory')
...
parser.add_argument('--dataset_list', type=str,
                    default=['testset_SharpFeature'], nargs='+',
                    help='list of .txt files containing sets of point cloud names for evaluation')                   
FLAGS = parser.parse_args()
EXPORT = False  # export some visualizations
PC_PATH = os.path.join(BASE_DIR, FLAGS.data_path)
PC_PATH_ALL_NORMALS = os.path.join(BASE_DIR, FLAGS.data_path_all_norm)
...
for dataset in dataset_list:
    ...
    normal_gt_filenames = PC_PATH + dataset.lower() + '.txt'
    xyz_path = PC_PATH
    normal_gt_path = PC_PATH_ALL_NORMALS
    ...
    for i, shape in enumerate(shape_names):
        ...
        # load the data
        points = np.loadtxt(os.path.join(xyz_path, shape + '.xyz')).astype('float32')

        # normals_gt = np.loadtxt(os.path.join(normal_gt_path, shape + '.normals')).astype('float32')
        try:
            normals_gt = loadmat(os.path.join(normal_gt_path, shape + '_true_normal.mat'))
            normals_gt = normals_gt['true_normals'][0]
            # print(normals_gt.shape)

        except:
            tmp_path = os.path.join(normal_gt_path, shape + '_true_normal.mat')
            mat = h5py.File(tmp_path, 'r')
            mat_normal = mat['true_normals']
            normals_gt = []
            for i in range(mat_normal.shape[0]):
                tmp = np.array([[],[],[]])
                for j in range(mat_normal.shape[1]):
                    st = mat_normal[i][j]
                    nor = mat[st][:]
                    nor = nor.T
                    # tmp = np.insert(tmp, 0, values=nor, axis=0)

                    tmp = np.concatenate([tmp, nor], axis=1)
                normals_gt.append(tmp)
        ...
        normals = [] 
        for i in range(normals_results.shape[0]):
            mat = normals_gt[i].T
            mat_norm = l2_norm(mat)
            mat = np.divide(mat, (np.tile(np.expand_dims(mat_norm, axis=1), [1,3])))
            num = mat.shape[0]
            out_mat = np.tile(normals_results[i], [num, 1])
            normal_index = list(np.sum(np.multiply(out_mat, mat), axis=1, keepdims=True))

            normal_index = [abs(x.item()) for x in normal_index]
            # normal_index = [x.item() if x.item() > 0 else 1 for x in normal_index]

            normal_index = normal_index.index(min(normal_index))
            normals.append(mat[normal_index])
        normals_gt = np.array(normals)
        ...
        ang_t = [x if x < 10 else 90 for x in ang] # RMS_t 所需的角度计算方式
        ...
        # RMS_t
        rms_t.append(np.sqrt(np.mean(np.square(ang_t))))
    ...
    log_string('RMS_t (shape average): ' + str(avg_rms_t))

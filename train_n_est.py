...
def parse_arguments():
    ...
    parser.add_argument('--name', type=str, default='TrueWeight+jet_order_1+neighbor_96_residuedis_sigma_max_0.1mean_0.0025_selected_20_20_normalguid_6_pca_STN_yes_yes_consistency_loss_no', help='training run name')
...
def train_pcpnet(opt):
    ...
    for epoch in range(opt.nepoch):
        ...
        for train_batchind, data in train_enum:
            ...
            # forward pass
            pred, beta_pred, weights, trans, trans2, neighbor_normals = model(pts_feas)

            points = pts_feas[:,0:3,:]
            loss, n_loss, _, consistency_loss, normal_loss, weight_loss  = compute_loss(
                pred=pred, target=target, inputs = points,
                outputs=opt.outputs,
            ...
            log_string('[%s %d: %d/%d] %s loss: %f, loss_normal: %f, loss_weight: %f' % (opt.name, epoch,
                                                       train_batchind, train_num_batch-1, green('train'), loss.item(), normal_loss.item(), weight_loss.item()), log_file)
            ...
            train_writer.add_scalar('normal_loss', normal_loss.item(),
                                    (epoch + train_fraction_done) * train_num_batch * opt.batchSize)
            train_writer.add_scalar('weight_loss', normal_loss.item(),
                                    (epoch + train_fraction_done) * train_num_batch * opt.batchSize)
            ...
            while test_fraction_done <= train_fraction_done and test_batchind+1 < test_num_batch:
                ...
                points = pts_feas[:,0:3,:]
                loss, n_loss, err_angle, consistency_loss, normal_loss, weight_loss  = compute_loss(
                    pred=pred, target=target, inputs = points,
                    outputs=opt.outputs,
                    output_pred_ind=output_pred_ind,
                    output_target_ind=output_target_ind,
                ...
                # print info and update log file
                log_string('[%s %d: %d/%d] %s loss: %f, loss_normal: %f, loss_weight: %f' % (opt.name, epoch, train_batchind,
                                                           train_num_batch-1, blue('test'), loss.item(), normal_loss.item(), weight_loss.item()), log_file)
                ...
                test_writer.add_scalar('normal_loss', normal_loss.item(),
                                       (epoch + test_fraction_done) * train_num_batch * opt.batchSize)
                test_writer.add_scalar('weight_loss', normal_loss.item(),
                                    (epoch + train_fraction_done) * train_num_batch * opt.batchSize)
                test_writer.add_histogram('weights', weights.detach().cpu().numpy(),
                                           (epoch + test_fraction_done) * train_num_batch * opt.batchSize)
...
def compute_loss(pred, inputs, target, outputs, output_pred_ind, output_target_ind, output_loss_weight, normal_loss_type, arch,
    ...
    return loss, n_loss, err_angle, consistency_loss, normal_loss, weight_loss 
